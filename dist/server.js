...
async function startServer(mode = 'stdio') {
    ...
    const server = new mcp_js_1.McpServer({ name: 'facturahub', version: '1.10.0' }, { instructions });
    ...
    const httpServer = (0, node_http_1.createServer)(async (req, res) => {
        ...
        if (url.pathname === '/sse' && req.method === 'GET') {
            try {
                ...
                const server = new mcp_js_1.McpServer({ name: 'facturahub', version: '1.10.0' }, { instructions: await buildInstructions(user) });
                registerTools(server, user);
                await server.connect(transport);
