...
function registerTools(server: McpServer, user: any): void {
  server.tool(
    'create_invoice',
    `[FacturaHub] Create an invoice. You can specify the client by name, email, or ID — if the client does not exist yet, it will be created automatically.
...
Always offer the e-invoice step for PA users — don't wait for the user to remember.`,
  ...
  server.tool(
    'list_invoices',
    '[FacturaHub] List invoices. Filter by client name, status, or date range.',
    {
  ...
  server.tool(
    'get_invoice',
    '[FacturaHub] Get full details of an invoice',
    { invoiceId: z.string().describe('Invoice ID') },
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async ({ invoiceId }) => {
      try {
  ...
  server.tool(
    'mark_paid',
    '[FacturaHub] Mark an invoice as paid',
    { invoiceId: z.string().describe('Invoice ID') },
  ...
  server.tool(
    'create_client',
    '[FacturaHub] Create a new client (you can also just use create_invoice with a client name and it will auto-create)',
    {
  ...
  server.tool(
    'list_clients',
    '[FacturaHub] List all your clients',
  ...
  server.tool(
    'get_client',
    '[FacturaHub] Get a single client by ID with full details',
  ...
  server.tool(
    'update_client',
    '[FacturaHub] Update client details (name, email, company, taxId, country, address, phone)',
    {
      clientId: z.string().describe('Client ID'),
  ...
  server.tool(
    'get_context',
    '[FacturaHub] Financial summary: pending totals, overdue, monthly income, invoice lists',
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async () => {
      try {
  ...
  server.tool(
    'download_pdf',
    '[FacturaHub] Generate and return a PDF for an invoice',
    { invoiceId: z.string().describe('Invoice ID') },
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async ({ invoiceId }) => {
      try {
  ...
  server.tool(
    ...
    async ({ invoiceId, ...updates }) => {
      try {
        const data = await api<any>(`/api/invoices/${invoiceId}`, { method: 'PUT', body: JSON.stringify(updates) });
  ...
  server.tool(
    'register_expense',
    '[FacturaHub] Register an expense with category and VAT/BTW rate. The taxRate is critical for quarterly VAT declarations — it calculates how much VAT you can recover.',
    {
  ...
  server.tool(
    'get_profit_loss',
    '[FacturaHub] Get profit & loss report — how much you earned vs spent in a period',
    {
  ...
  server.tool(
    'get_cashflow',
    '[FacturaHub] Cash flow projection based on pending invoices and recurring expenses',
  ...
  server.tool(
    'list_expenses',
    '[FacturaHub] List all expenses with IDs, amounts, vendors, categories, and dates. Filter by category or date range.',
    {
    ...
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async ({ category, dateFrom, dateTo }) => {
      try {
        const params = new URLSearchParams();
        ...
        if (expenses.length === 0) return ok('No expenses found.');
  ...
  server.tool(
    'get_expense',
    '[FacturaHub] Get a single expense by ID with full details',
  ...
  server.tool(
    ...
    '[FacturaHub] Update an existing expense — correct amounts, dates, categories, vendors, tax rates, or add notes',
    {
      ...
      category: z.enum(['software', 'hosting', 'travel', 'office', 'professional_services', 'marketing', 'taxes', 'salary', 'equipment', 'other']).optional(),
  ...
  server.tool(
    'delete_expense',
    '[FacturaHub] Delete an expense by ID',
  ...
  server.tool(
    'get_vat_balance',
    '[FacturaHub] Get your VAT/IVA/BTW balance — how much tax you collected vs paid, and whether you owe or have credit. Essential for quarterly tax declarations.',
    {
  ...
  server.tool(
    'get_income_summary',
    '[FacturaHub] Annual income/earnings summary (ganancias) — gross income, net income, expenses, taxable income, with quarterly breakdown. Use this for income tax planning.',
    {
      year: z.number().optional().describe('Year. Defaults to current year.'),
    },
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
  ...
  server.tool(
    'get_profile',
    '[FacturaHub] Get your business profile — company info, address, tax details, bank info, currency, and plan',
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async () => {
      try {
  ...
  server.tool(
    'update_profile',
    '[FacturaHub] Update your business profile — company name, address, tax ID, bank details, currency, tax rate, etc.',
    {
      company: z.string().optional().describe('Company or business name'),
      taxId: z.string().optional().describe('Tax ID (NIF, CIF, RFC, CUIT, etc.)'),
      country: z.string().optional().describe('Country code (e.g. ES, MX, US, AR)'),
      currency: z.string().optional().describe('Currency code (EUR, USD, MXN, ARS, etc.)'),
      taxRate: z.number().optional().describe('Default tax rate % (e.g. 21 for 21%)'),
      address: z.string().optional().describe('Street address'),
      city: z.string().optional().describe('City'),
      postalCode: z.string().optional().describe('Postal / ZIP code'),
      phone: z.string().optional().describe('Phone number'),
      bankName: z.string().optional().describe('Bank name'),
      bankIban: z.string().optional().describe('IBAN or account number'),
      bankSwift: z.string().optional().describe('SWIFT / BIC code'),
      bankAccountHolder: z.string().optional().describe('Account holder name'),
    },
    { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    async (params) => {
      try {
  ...
  // =========================================================================
  server.tool(
    'validate_invoice_panama',
    `[FacturaHub] Validate a Panamanian invoice: checks RUC and verifies arithmetic (ITBMS). Use before sending to DGI.

After validation passes, offer: "¿Quieres que envíe esta factura a la DGI?"`,
    {
      invoiceId: z.string().optional().describe('FacturaHub invoice ID.'),
      seller_ruc: z.string().optional().describe('Seller RUC (e.g. 1-184-921). Required if no invoiceId.'),
      buyer_ruc: z.string().optional().describe('Buyer RUC. Required if no invoiceId.'),
      lines: z.array(z.object({
        description: z.string(),
        qty: z.number(),
        unit_price: z.number(),
        tax_rate: z.number().describe('ITBMS rate: 0, 7, 10, or 15'),
    ...
    async (params) => {
      try {
        if (params.invoiceId) {
          ...
          if (result.errors.length > 0) { lines.push(``, `Errores:`); result.errors.forEach((e: string) => lines.push(`  - ${e}`)); }
          return ok(lines.join('\n'));
        }
  ...
  server.tool(
    'submit_to_dgi',
    `[FacturaHub] Submit a FacturaHub invoice to DGI Panama via Factura Facil PAC. Generates FE XML, signs, and obtains CUFE/CAFE.`,
    {
      invoiceId: z.string().describe('FacturaHub invoice ID to submit'),
    },
    { readOnlyHint: false, destructiveHint: false, openWorldHint: true },
    async ({ invoiceId }) => {
      try {
  ...
  server.tool(
    'create_task',
    '[FacturaHub] Create a new task on the team board. Tasks start in "backlog" status. Can assign to a team member and link to invoices or clients.',
    {
      title: z.string().describe('Task title'),
      description: z.string().optional().describe('Detailed description'),
      priority: z.enum(['low', 'medium', 'high', 'urgent']).optional().describe('Priority level. Defaults to medium.'),
      assignedTo: z.string().optional().describe('User ID of the team member to assign to'),
      dueDate: z.string().optional().describe('Due date YYYY-MM-DD'),
      tags: z.array(z.string()).optional().describe('Tags for categorization (e.g. ["bug", "frontend"])'),
      linkedInvoiceId: z.string().optional().describe('Link to an invoice ID'),
      linkedClientId: z.string().optional().describe('Link to a client ID'),
    },
    { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
    async (params) => {
      try {
  ...
  server.tool(
    'list_tasks',
    '[FacturaHub] List tasks with optional filters by status, assignee, priority, or tag.',
    {
    ...
    async (params) => {
      try {
        ...
        if (!tasks.length) return ok('No tasks found with these filters.');
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        if (!tasks.length) return ok('No tasks assigned to you.');
  ...
  server.tool(
    'get_task_overview',
    '[FacturaHub] Get a high-level overview of the task board: counts by status and by assignee.',
    {},
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async () => {
      try {
        ...
        const statusLines = Object.entries(data.byStatus || {}).map(
          ([status, count]) => `  ${status}: ${count}`
        );
        const assigneeLines = (data.byAssignee || []).map(
          (a: any) => `  ${a.name || a.email}: ${a.count} tasks`
        );
  ...
  server.tool(
    'move_task',
    `[FacturaHub] Move a task to a new status. Valid transitions:
• backlog → todo
...
• done → (no transitions)`,
    {
      ...
      status: z.enum(['backlog', 'todo', 'in_progress', 'review', 'done']).describe('Target status'),
    },
    { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
    async ({ taskId, status }) => {
      try {
  ...
  server.tool(
    'assign_task',
    '[FacturaHub] Assign a task to a team member.',
    {
      taskId: z.string().describe('Task ID'),
      assignedTo: z.string().describe('User ID of the team member'),
    },
    { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
  ...
  server.tool(
    'update_task',
    '[FacturaHub] Update task fields (title, description, priority, due date, tags).',
    {
      taskId: z.string().describe('Task ID'),
  ...
  server.tool(
    'complete_task',
    '[FacturaHub] Mark a task as done. The task must be in "review" status.',
    {
      taskId: z.string().describe('Task ID'),
    },
    { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
  ...
  server.tool(
    'delete_task',
    '[FacturaHub] Delete a task. Only tasks in "backlog" or "todo" status can be deleted.',
    {
      taskId: z.string().describe('Task ID'),
    },
    { readOnlyHint: false, destructiveHint: true, openWorldHint: false },
  ...
  server.tool(
    ...
    '[FacturaHub] Get the living documentation of this business — what the company does, clients, financial patterns, operations, and team. Always read this at the start of a conversation to understand the full context.',
    {},
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async () => {
      try {
        const data = await api<{ markdown: string; entryCount: number }>('/api/context/markdown');
        if (!data.markdown) {
  ...
  server.tool(
    ...
    '[FacturaHub] Save or update a piece of business knowledge. Use this PROACTIVELY whenever you learn something new about the business — a new client pattern, financial insight, operational process, product/service detail, or team structure. This builds the living documentation that makes you smarter over time.',
    {
      key: z.string().describe('Unique identifier for this entry (e.g., "client:acme", "pattern:monthly-saas", "process:invoice-approval"). Use lowercase with colons.'),
      title: z.string().describe('Human-readable title (e.g., "Acme Corp - Main Client", "Monthly SaaS Revenue Pattern")'),
      content: z.string().describe('Detailed description in natural language. Include numbers, dates, and specifics. Write as documentation that your future self will read.'),
      category: z.enum(['company', 'clients', 'financial', 'operations', 'products', 'team', 'notes']).describe('Category: company (profile, mission), clients (relationships, patterns), financial (revenue, expenses, patterns), operations (processes, workflows), products (what they sell/do), team (structure, roles), notes (misc)'),
    },
    { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
    async ({ key, title, content, category }) => {
      try {
        const data = await api<{ message: string }>('/api/context/entry', {
          method: 'PUT',
  ...
  server.tool(
    'remove_business_context',
    '[FacturaHub] Remove an outdated or incorrect business context entry.',
    {
      key: z.string().describe('The key of the entry to remove'),
    },
    { readOnlyHint: false, destructiveHint: true, openWorldHint: false },
    async ({ key }) => {
      try {
  ...
  server.tool(
    ...
    '[FacturaHub] Update the executive summary of the business. This is the first thing read in every conversation — a 2-3 paragraph overview of who they are, what they do, and their current situation.',
    {
      summary: z.string().describe('Executive summary of the business in 2-3 paragraphs. Include: what the company does, main revenue sources, team size, current priorities.'),
    },
    { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
    async ({ summary }) => {
      try {
  ...
  server.tool(
    ...
    '[FacturaHub] Generate a tax declaration for a period. Supported countries: NL (BTW quarterly), ES (Modelo 303 quarterly), PA (ITBMS bimonthly), MX (IVA monthly), CO (IVA bimonthly).',
    {
      country: z.string().describe('Country code: NL, ES, PA, MX, or CO'),
      year: z.number().describe('Tax year'),
      period: z.number().describe('Period index (1-based): quarter for NL/ES, bimonth for PA/CO, month for MX'),
    },
    async ({ country, year, period }) => {
      try {
        const result = await api<any>('/api/tax-declarations/generate', {
          method: 'POST',
        ...
        if (d.warnings?.length) {
          lines.push('', '**Warnings**', ...d.warnings.map((w: string) => `  - ${w}`));
        }
  ...
  server.tool(
    ...
    '[FacturaHub] List tax declarations with optional filters.',
    {
    ...
    async ({ country, year }) => {
      try {
        ...
        if (year) params.set('year', String(year));
        const result = await api<any>(`/api/tax-declarations?${params}`);
        if (!result.declarations?.length) {
          return ok('No tax declarations found.');
        }
        const lines = result.declarations.map((d: any) =>
          `- [${d.status}] ${d.formName} — ${d.periodLabel} | Net: ${d.totals.netTax} ${d.totals.currency} (ID: ${d._id})`
        );
        return ok(lines.join('\n'));
      } catch (e: unknown) {
  ...
  server.tool(
    'get_tax_declaration',
    '[FacturaHub] Get full details of a specific tax declaration.',
    {
      id: z.string().describe('Tax declaration ID'),
    },
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async ({ id }) => {
      try {
        const result = await api<any>(`/api/tax-declarations/${id}`);
  ...
  server.tool(
    ...
    '[FacturaHub] Update manual overrides or notes on a draft tax declaration.',
    {
      id: z.string().describe('Tax declaration ID'),
      manualOverrides: z.record(z.string(), z.any()).optional().describe('Manual field overrides'),
      notes: z.string().optional().describe('Notes about this declaration'),
    },
  ...
  server.tool(
    ...
    async ({ id }) => {
      try {
        const result = await api<any>(`/api/tax-declarations/${id}/submit`, { method: 'PATCH' });
  ...
  server.tool(
    'list_catalog_items',
    '[FacturaHub] List catalog items (products and services). Filter by type, category, or search by name.',
    {
      type: z.enum(['product', 'service']).optional().describe('Filter by item type'),
      category: z.string().optional().describe('Filter by category'),
      search: z.string().optional().describe('Search by name (partial match)'),
      includeInactive: z.boolean().optional().describe('Include inactive/deactivated items (default false)'),
    },
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async ({ type, category, search, includeInactive }) => {
      try {
        const params = new URLSearchParams();
        ...
        if (items.length === 0) return ok('No catalog items found.');
  ...
  server.tool(
    'create_catalog_item',
    '[FacturaHub] Create a new product or service in the catalog. Products can track inventory with stock levels and low-stock alerts.',
    {
      name: z.string().describe('Item name'),
      type: z.enum(['product', 'service']).describe('Item type: product (physical good) or service'),
      unitPrice: z.number().describe('Sale price per unit'),
      description: z.string().optional().describe('Item description'),
      sku: z.string().optional().describe('SKU code (must be unique if provided)'),
      barcode: z.string().optional().describe('Barcode (EAN, UPC, etc.)'),
      costPrice: z.number().optional().describe('Cost price (what you pay the supplier)'),
      currency: z.string().optional().describe('Currency code. Defaults to user currency.'),
      taxRate: z.number().optional().describe('Tax rate % for this item'),
      category: z.string().optional().describe('Category for grouping (e.g. "electronics", "consulting")'),
      trackInventory: z.boolean().optional().describe('Enable stock tracking (default false)'),
      stock: z.number().optional().describe('Initial stock quantity (default 0)'),
      lowStockAlert: z.number().optional().describe('Stock threshold to trigger low-stock alert'),
    },
    { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
    async (params) => {
      try {
  ...
  server.tool(
    'update_catalog_item',
    '[FacturaHub] Update an existing catalog item — change name, price, description, category, stock settings, etc.',
    {
      itemId: z.string().describe('Catalog item ID'),
      name: z.string().optional().describe('New name'),
      type: z.enum(['product', 'service']).optional().describe('New type'),
      unitPrice: z.number().optional().describe('New sale price'),
      description: z.string().optional().describe('New description'),
      ...
      taxRate: z.number().optional().describe('New tax rate %'),
      category: z.string().optional().describe('New category'),
      trackInventory: z.boolean().optional().describe('Enable/disable stock tracking'),
      stock: z.number().optional().describe('Set stock quantity directly'),
      lowStockAlert: z.number().optional().describe('New low-stock alert threshold'),
  ...
  server.tool(
    'adjust_stock',
    '[FacturaHub] Adjust inventory stock for a catalog item. Use positive numbers to add stock (restock) and negative numbers to remove stock (damage, loss, correction). For sales, use add_movement instead.',
    {
      itemId: z.string().describe('Catalog item ID'),
      adjustment: z.number().describe('Stock adjustment: positive to add, negative to remove (e.g. 10 to add 10 units, -3 to remove 3)'),
    },
    { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
  ...
  server.tool(
    'list_vendors',
    '[FacturaHub] List all vendors/suppliers. Search by name, email, or category.',
    {
      search: z.string().optional().describe('Search by name, email, or category (partial match)'),
    },
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async ({ search }) => {
      try {
  ...
  server.tool(
    ...
    '[FacturaHub] Create a new vendor/supplier. Vendors are linked to expenses for tracking who you pay.',
    {
      ...
      phone: z.string().optional().describe('Phone number'),
      taxId: z.string().optional().describe('Tax ID (NIF, RUC, RFC, etc.)'),
      address: z.string().optional().describe('Address'),
      country: z.string().optional().describe('Country code'),
      category: z.string().optional().describe('Category (e.g. "office supplies", "cloud services")'),
      notes: z.string().optional().describe('Notes about this vendor'),
    },
    { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
    async (params) => {
      try {
  ...
  server.tool(
    'get_open_register',
    '[FacturaHub] Get the currently open cash register. Returns null if no register is open. Use this to check POS status before adding sales.',
    {},
    { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async () => {
      try {
  ...
  server.tool(
    'add_movement',
    '[FacturaHub] Add a sale (or other movement) to the open cash register. For sales, you can include line items with product details. Consumer-final sales are grouped into a single invoice when the register is closed.',
    {
      registerId: z.string().describe('Cash register ID (get from get_open_register)'),
      type: z.enum(['sale', 'refund', 'cash_in', 'cash_out']).describe('Movement type: sale (customer purchase), refund, cash_in (manual cash deposit), cash_out (manual cash withdrawal)'),
      amount: z.number().describe('Total amount of the movement (must be positive)'),
      description: z.string().describe('Description of the movement (e.g. "2x Coffee, 1x Sandwich")'),
      consumerFinal: z.boolean().optional().describe('Mark as consumer-final sale (anonymous customer). These are grouped into one invoice on register close.'),
      lineItems: z.array(z.object({
        description: z.string().describe('Line item description'),
        quantity: z.number().describe('Quantity sold'),
        unitPrice: z.number().describe('Unit price'),
        taxRate: z.number().optional().describe('Tax rate % for this item'),
        catalogItemId: z.string().optional().describe('Catalog item ID for stock tracking'),
      })).optional().describe('Detailed line items for the sale. If provided, stock will be adjusted for catalog items.'),
    },
    { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
    async ({ registerId, type, amount, description, consumerFinal, lineItems }) => {
      try {
  ...
  server.tool(
    ...
    {
      status: z.enum(['pending', 'partial', 'paid', 'overdue', 'cancelled']).optional().describe('Filter by payment status'),
    ...
    async ({ status, limit }) => {
      try {
        ...
        const data = await api<any[]>(`/api/supplier-invoices?${params}`);
        if (!data.length) return ok('No supplier invoices found.');
        const lines = data.map((inv: any) =>
          `- ${inv.invoiceNumber} | ${inv.vendorId?.name || '?'} | ${formatMoney(inv.total, inv.currency)} | status: ${inv.status} | balance: ${formatMoney(inv.balance, inv.currency)}`
        );
        return ok(`${data.length} supplier invoice(s):\n${lines.join('\n')}`);
      } catch (e: unknown) { return err(`Error: ${e instanceof Error ? e.message : String(e)}`); }
    },
  );
  ...
  server.tool(
    ...
    {
      vendorName: z.string().describe('Vendor name (auto-finds or creates)'),
      invoiceNumber: z.string().describe('Supplier invoice number'),
      total: z.number().describe('Total amount'),
      currency: z.string().optional().describe('Currency (default: user currency)'),
      dueDate: z.string().optional().describe('Due date YYYY-MM-DD (default: 30 days)'),
      taxRate: z.number().optional().describe('Tax rate % (default 0)'),
      notes: z.string().optional(),
    },
    { readOnlyHint: false },
    async ({ vendorName, invoiceNumber, total, currency, dueDate, taxRate, notes }) => {
      try {
        const result = await api<any>('/api/supplier-invoices', {
          method: 'POST',
  ...
  server.tool(
    ...
    {
      invoiceId: z.string().describe('Supplier invoice ID'),
    },
    { readOnlyHint: true },
    async ({ invoiceId }) => {
      try {
        const inv = await api<any>(`/api/supplier-invoices/${invoiceId}`);
        const payments = (inv.payments || []).map((p: any) =>
          `  - ${formatDate(p.paymentDate)}: ${formatMoney(p.amount, inv.currency)} via ${p.paymentMethod}`
        );
        return ok([
  ...
  server.tool(
    ...
    '[FacturaHub] Record a payment against a supplier invoice.',
    {
      ...
      paymentMethod: z.enum(['transfer', 'cash', 'check', 'card', 'other']).optional().describe('Payment method (default: transfer)'),
      reference: z.string().optional().describe('Payment reference/transaction ID'),
      notes: z.string().optional(),
    },
    { readOnlyHint: false },
    async ({ invoiceId, amount, paymentMethod, reference, notes }) => {
      try {
        const result = await api<any>(`/api/supplier-invoices/${invoiceId}/payments`, {
          method: 'POST',
  ...
  server.tool(
    ...
    async () => {
      try {
        const teams = await api<any[]>('/api/teams/my');
        if (!teams.length) return ok('You are not a member of any team.');
        const lines = teams.map((t: any) => `- ${t.name} (${t.role}) — ${t.memberCount || '?'} members`);
  ...
  server.tool(
    'list_team_members',
    '[FacturaHub] List members of the current active team.',
    {},
    { readOnlyHint: true },
    async () => {
      try {
        const members = await api<any[]>('/api/teams/members');
        if (!members.length) return ok('No team members found.');
  ...
  server.tool(
    'invite_team_member',
    '[FacturaHub] Invite a new member to the current team by email.',
    {
      email: z.string().describe('Email of the person to invite'),
      role: z.enum(['admin', 'finance', 'operations', 'member', 'viewer']).describe('Role to assign'),
    },
    { readOnlyHint: false },
    async ({ email, role }) => {
      try {
  ...
  server.tool(
    'remove_team_member',
    '[FacturaHub] Remove a member from the current team. Requires admin or owner role.',
    {
      memberId: z.string().describe('Team member ID to remove'),
    },
  ...
  server.tool(
    ...
    '[FacturaHub] Change the role of a team member. Requires admin or owner role.',
    {
      ...
      role: z.enum(['admin', 'finance', 'operations', 'member', 'viewer']).describe('New role to assign'),
    },
    { readOnlyHint: false },
    async ({ memberId, role }) => {
      try {
        const member = await api<any>(`/api/teams/members/${memberId}/role`, {
          method: 'PATCH',
  ...
  server.tool(
    ...
    '[FacturaHub] Get a comprehensive dashboard summary with KPIs: total revenue, outstanding amount, expenses, net profit, overdue invoices, and recent activity counts. Perfect for a quick business overview.',
    {
      period: z.enum(['month', 'quarter', 'year']).optional().describe('Time period for the summary. Defaults to current month.'),
    },
    { readOnlyHint: true },
    async ({ period }) => {
      try {
        const qs = period ? `?period=${period}` : '';
        const summary = await api<any>(`/api/dashboard/summary${qs}`);
        return ok([
  ...
  server.tool(
    'get_activity_log',
    '[FacturaHub] Get the recent activity/audit log. Shows who did what and when — useful for team oversight and accountability.',
    {
      limit: z.number().optional().describe('Number of entries to return (default 20, max 100)'),
      action: z.string().optional().describe('Filter by action type (e.g. "invoice.created", "expense.registered", "task.completed")'),
    },
    { readOnlyHint: true },
    async ({ limit, action }) => {
      try {
        ...
        const qs = params.toString() ? `?${params.toString()}` : '';
        const logs = await api<any[]>(`/api/activity${qs}`);

        if (!logs.length) return ok('No activity found.');
  ...
  server.tool(
    'get_aging_report',
    '[FacturaHub] Get accounts receivable aging report — breaks down unpaid invoices by how overdue they are (current, 1-30 days, 31-60, 61-90, 90+). Essential for cash flow management.',
    {},
    { readOnlyHint: true },
    async () => {
      try {
        const aging = await api<any>('/api/reports/aging');
  ...
  server.tool(
    ...
    '[FacturaHub] Get revenue breakdown by client — shows how much each client has paid and owes. Useful for identifying top clients and concentration risk.',
    {
      dateFrom: z.string().optional().describe('From date YYYY-MM-DD'),
      dateTo: z.string().optional().describe('To date YYYY-MM-DD'),
      limit: z.number().optional().describe('Max number of clients to return (default all)'),
    },
    { readOnlyHint: true },
    async ({ dateFrom, dateTo, limit }) => {
      try {
        ...
        if (limit) params.set('limit', String(limit));
        const qs = params.toString() ? `?${params.toString()}` : '';
        const report = await api<any>(`/api/reports/revenue-by-client${qs}`);

        const clients = report.clients || report;
        if (!Array.isArray(clients) || !clients.length) return ok('No revenue data found for this period.');

        const c = report.currency || clients[0]?.currency || 'EUR';
        const lines = clients.map((cl: any, i: number) => {
        ...
        const totalRevenue = clients.reduce((s: number, cl: any) => s + (cl.revenue || cl.total || 0), 0);
        return ok([
          `Revenue by Client:`,
          ``,
          ...lines,
  ...
  server.tool(
    'export_data',
    '[FacturaHub] Export invoices or expenses as CSV. Returns a download link.',
    {
      type: z.enum(['invoices', 'expenses']).describe('What to export'),
      dateFrom: z.string().optional().describe('From date YYYY-MM-DD'),
      dateTo: z.string().optional().describe('To date YYYY-MM-DD'),
      status: z.string().optional().describe('Filter by status (e.g. "paid", "draft")'),
    },
    { readOnlyHint: true, openWorldHint: true },
    async ({ type, dateFrom, dateTo, status }) => {
      try {
        const params = new URLSearchParams();
        if (dateFrom) params.set('dateFrom', dateFrom);
        if (dateTo) params.set('dateTo', dateTo);
        if (status) params.set('status', status);
        const qs = params.toString() ? `?${params.toString()}` : '';
        const result = await api<any>(`/api/export/${type}${qs}`);
  ...
  server.tool(
    'get_billing_info',
    '[FacturaHub] Get current plan and billing status.',
    {},
    { readOnlyHint: true },
    async () => {
      try {
        const billing = await api<any>('/api/billing/status');
        return ok([
  ...
  server.tool(
    ...
    {
      plan: z.enum(['starter', 'growth', 'pro', 'business']).describe('Target plan'),
    },
  ...
  server.tool(
    ...
    { readOnlyHint: false },
    async ({ vendorId, ...fields }) => {
      try {
        const vendor = await api<any>(`/api/vendors/${vendorId}`, {
          method: 'PATCH',
  ...
  server.tool(
    'open_register',
    '[FacturaHub] Open a new cash register session with initial cash amount.',
    {
      openingCash: z.number().describe('Initial cash in register'),
      currency: z.string().optional().describe('Currency (default: user currency)'),
    },
    { readOnlyHint: false },
    async ({ openingCash, currency }) => {
      try {
        const register = await api<any>('/api/cash-register/open', {
          method: 'POST',
  ...
  server.tool(
    ...
    {
      registerId: z.string().describe('Cash register ID'),
      closingCash: z.number().describe('Actual cash counted in register'),
    },
    { readOnlyHint: false },
    async ({ registerId, closingCash }) => {
      try {
        const result = await api<any>(`/api/cash-register/${registerId}/close`, {
          method: 'POST',
...
async function buildInstructions(user: any): Promise<string> {
  // Try to load business context
  let businessCtx = '';
  try {
    const ctx = await api<{ markdown: string; entryCount: number }>('/api/context/markdown');
    if (ctx.markdown) {
  ...
  let roleInfo = '';
  try {
    const role = await api<{ role: string; teamId?: string }>('/api/teams/my-role');
    if (role.role) {
...
export async function startServer(mode: 'stdio' | 'http' = 'stdio'): Promise<void> {
  if (mode === 'http') {
  ...
  const server = new McpServer(
    { name: 'facturahub', version: '1.10.0' },
    { instructions },
  );
...
async function startHttpServer(): Promise<void> {
  ...
  function extractApiKey(req: import('node:http').IncomingMessage, url: URL): string | null {
